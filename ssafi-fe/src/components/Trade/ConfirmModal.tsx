...
const Btn = styled.div<{check: boolean}>`
  ...
  background: ${(props) => (props.check ? 'var(--point-color)' : 'var(--light-gray-color)')};
  &.cancle {
    cursor: pointer;
  }
`;

const ConfirmModal = ({ inputData, closeModal, setIsTrade }: ConfirmModalProps) => {
  const {
    safetyRatio, neutralRatio, riskRatio, aiBudget, aiGoal,
  } = inputData;
      ...
      <ModalContainer>
        ...
        <Info>
          ...
          <RowContainer>
            ...
            <Text>: 안전({safetyRatio}%), 중립({neutralRatio}%), 위험({riskRatio}%)</Text>
          </RowContainer>
          <RowContainer>
            <Text className='bold'>투자금액</Text>
            <Text>: {aiBudget}원 ({convertToKoreannum(aiBudget, '투자')})</Text>
          </RowContainer>
          <RowContainer>
            <Text className='bold'>목표금액</Text>
            <Text>: {aiGoal}원 ({convertToKoreannum(aiGoal, '목표')})</Text>
          </RowContainer>
        </Info>
        <Info>
          <SubTitle>안내: AI 트레이딩</SubTitle>
          <Content>
            <Text>AI 트레이딩 전략은 입력해주신 종목 투자 비율을 바탕으로 진행되며,</Text>
            <Text>목표 금액을 달성하면 트레이딩은 자동으로 종료됩니다.</Text><div style={{ height: '8px' }}/>
            <Text>AI 트레이딩은 금융 시장의 변동성에 영향을 받으며, 수익이나 손실이 발생할 수 있습니다.</Text>
            <Text>트레이딩을 시작하기 전에 투자 비율과 자본을 신중하게 고려하십시오.</Text>
            <Text>투자 정보를 변경하려면 취소 버튼을 눌러 설정을 수정하십시오.</Text><div style={{ height: '8px' }}/>
            <Text>투자 성과와 목표 달성 여부를 실시간으로 확인할 수 있습니다.</Text>
            <Text>만약, 트레이딩을 중지하려면 투자 중지하기 버튼을 클릭하십시오.</Text>
          </Content>
        </Info>
        <RowContainer>
          <CheckBox type='checkbox' checked={isCheck} onChange={() => setIsCheck(!isCheck)}/>
          <Text>위 사항을 모두 숙지하였습니다.</Text>
        </RowContainer>
        <BtnContainer>
          <Btn className='cancle' check={false} onClick={() => closeModal(false)}>취소</Btn>
          <Btn check={isCheck} onClick={() => handleStartBtn(isCheck)}>시작하기</Btn>
        </BtnContainer>
      </ModalContainer>
    </ModalBackground>
  );
